...
to go
  ...
  if ticks mod number-of-ticks-per-day != 0 and (ticks + 1) mod number-of-ticks-per-day != 0 [
  ...
  if (ticks + 1) mod number-of-ticks-per-day = 0 [ ;; Last tick of a day
    ...
    ;; Triggers the exogenous infection
    Randomly_infect_workers_exogenously

    ;; Plot all the relevant variables
    plot_vars

    ;; Updates the workers immunity
    immunity_update

    ;; Brings recovered worker back to work
    recovery_of_sick_workers
  ]
...
to Randomly_infect_workers_exogenously
  ;; Randomly infect workers exogenously
  let endogenous-incidence count turtles with [infected = true] ;; For plot
  ask turtles with [sick = false and immune = 0] [
    set infected infected or random-float 1 < exogenous-sickness ;; Some healthy and vulnerable workers may be infected already
...
Stonedahl, F. and Wilensky, U. (2008). http://ccl.northwestern.edu/netlogo/models/VirusonaNetwork. Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
