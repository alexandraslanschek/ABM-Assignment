...
to spread-virus ; thsi function is partially replicated from the virus model
  ask turtles with [infected?][
    ask link-neighbors with [not home?][
      if tolerance = 1 [
        if random-float 1 < high-spread-chance [
           become-infected  ] ]
      if tolerance = 2 [
...
to get-better
  ; the way i implemented get-better does not distinguishes the difference between recovering at home or in the office.
  ; it can be changed afterwards
  ask turtles with [infected?][
    if ticks-since-infection >= recovery-time [
      become-healthy ]
...
to infection-time-counter
  ; here we need to create a tick counter. It need to be either at the beginning or the end of the
  ask turtles with [ infected? ] [
...
to start-outbreak
    ask n-of initial-infections number-of-agents ; this is replicated from the virus model
    [ become-infected ]
end
